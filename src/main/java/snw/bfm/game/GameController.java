...
import snw.bfm.tasks.ScheduledRFMTask;
import snw.bfm.tasks.ScheduledRFMTaskImpl;
...
public final class GameController implements snw.rfm.api.GameController {
    ...
    public void addMoney(String player, double coin) {
        ...
        Map<String, Double> ce = BattleForMoney.getInstance().getCoinEarned();
    ...
    public void forceOut(Player player) throws IllegalStateException {
        ...
        Map<String, Double> earned = BattleForMoney.getInstance().getCoinEarned(); // 2022/2/2 有现成的 get 我不用。。。
    ...
    public Double getMoney(String player) {
        return BattleForMoney.getInstance().getCoinEarned().get(player);
    }

    @Override
    public void setMoney(String player, double amount) {
        BattleForMoney.getInstance().getCoinEarned().put(player, amount);
    }
}
